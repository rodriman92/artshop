...
const mostrarCarrito = () => {
  ...
  carrito.forEach((element) => {
    let { imagenColeccion, descripcionImagen, cantidad, precio, indice } =
      element;
...
const borrarColeccion = (coleccionBorrar) => {
  carrito = carrito.filter(
    (element) => element.indice !== Number(coleccionBorrar)
  );
  mostrarCarrito();
};
    ...
    colecciones.forEach((coleccion) => {
      ...
      fetch("https://criptoya.com/api/bitex/" + moneda + "/usd/1")
        ...
        .then((data) => {
          ...
          tbody.innerHTML += `
            <tr class="tr">
                <td class="td col-xs-2" style="width: 20%">${
                  coleccion.descripcionImagen
                }</td>
                <td class="td col-xs-2" style="width: 25%">${
                  coleccion.autorImagen
                }</td>
                <td class="td col-xs-2" style="width: 20%">${Math.round(
                  valorEnDolares * coleccion.precio
                ).toFixed(2)}</td>
                <td class="td col-xs-2" style="width: 15%">${
                  coleccion.precio
                } </td>
...
const calcularTotal = () => {
  ...
  divTotal.innerHTML = `
        ...
        <td colspan="2" class="td tdTotal"><span class="spanTotal">US$${Math.round(
          total
        ).toFixed(3)}</span></td>
    `;
    ...
    e.addEventListener("click", (e) => {
      ...
      function handleSubmit(event) {
        ...
        $buttonMailto.setAttribute(
          "href",
          `mailto:contacto@artshop.com?subject=Consulta NFTs&body=Mensaje nuevo de ${form.get(
            "name"
          )}. Consulta: ${form.get("message")}`
        );
        $buttonMailto.click();
        setTimeout(function () {
